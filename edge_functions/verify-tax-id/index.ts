// =============================================================================
// WIMRUX® FINANCES — Edge Function : verify-tax-id
...
interface VerifyRequest {
  country_code: string;   // ISO 3166-1 alpha-2 ex: "BF"
  tax_id: string;         // IFU / NIF / TIN à vérifier
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (configErr || !config) {
      // Pays inconnu : validation générique
      return new Response(
        JSON.stringify({
          country_code,
          tax_id,
    ...
    // 1. Validation format local (regex)
    ...
    // 2. Vérification en ligne selon le type
    ...
    if (config.verification_type === "web_scrape" && config.verification_url && format_valid) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Chercher des indicateurs positifs ou négatifs dans le HTML
          ...
          if (validPattern.test(html)) {
            ...
            // Réponse HTML reçue mais résultat ambigu
      ...
      // Futur : API JSON pays (Sénégal, Ghana, Nigeria...)
      online_check = "not_available";
