// ─── SMTP via nodemailer (Deno compatible via npm:) ───────────────────────────
import nodemailer from "npm:nodemailer@6.9.13";

const SMTP_HOST = Deno.env.get("SMTP_HOST") ?? "vmi2335626.contaboserver.net";
const SMTP_PORT = parseInt(Deno.env.get("SMTP_PORT") ?? "465");
const SMTP_USER = Deno.env.get("SMTP_USER") ?? "noreply@wimrux.app";
const SMTP_PASS = Deno.env.get("SMTP_PASS") ?? "";

// ─── Logo WIMRUX en base64 inline (W blanc sur #0F172A, 120x120) ─────────────
// On utilise une URL publique de l'image déployée sur Vercel
...
function getTemplate(name: string, vars: Record<string, string>): { subject: string; bodyHtml: string; preheader?: string } {
  switch (name) {
    case "otp": {
      return {
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const templateName = (body.template as string) ?? "custom";
  const vars = (body.vars as Record<string, string>) ?? {};
  const customSubject = body.subject as string | undefined;
  const customHtmlBody = body.html_body as string | undefined;

  if (!to) {
    return new Response(JSON.stringify({ error: "Missing 'to' field" }), { status: 400 });
  }
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: "SMTP send failed", detail: message }), {
      status: 500,
