import paramiko
import time

HOST = '167.86.69.104'
USER = 'root'
PWD = 'JVQ9UU3G7nrm'

client = paramiko.SSHClient()
client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
client.connect(HOST, username=USER, password=PWD)

project_id = "6a29285200015cd421c7"
api_key = ""

# D'abord trouver le resourceInternalId de la fonction
stdin, stdout, stderr = client.exec_command(
    "docker exec appwrite-mariadb mysql -u root -p'appwrite-db-pass-2024' appwrite -e 'SELECT _uid, _id, internalId FROM _1_functions WHERE _id=\"send-otp-whatsapp\"'"
)
func_info = stdout.read().decode()
print(f"Function info:\n{func_info}")

# Créer les variables de la fonction
vars_to_set = [
    ("APPWRITE_PROJECT", project_id),
    ("APPWRITE_API_KEY", api_key),
    ("WHAPI_TOKEN", "7oUdVCMwhatfvWgRZJBner9bKvCfPq9t"),  # Token de la mémoire InsForge
    ("APPWRITE_ENDPOINT", "https://appwrite.benga.live/v1"),
    ("APPWRITE_DATABASE", "wimrux_finances"),
]

for key, value in vars_to_set:
    uid = f"var_{key.lower()}_{project_id[:8]}"
    now = int(time.time())
    
    stdin, stdout, stderr = client.exec_command(
        f"docker exec appwrite-mariadb mysql -u root -p'appwrite-db-pass-2024' appwrite -e "
        f"'INSERT INTO _1_variables (_uid, _createdAt, _updatedAt, resourceType, resourceId, key, value, search) "
        f"VALUES (\"{uid}\", FROM_UNIXTIME({now}), FROM_UNIXTIME({now}), \"functions\", \"send-otp-whatsapp\", \"{key}\", \"{value}\", \"{key} {value}\") "
        f"ON DUPLICATE KEY UPDATE value=\"{value}\", _updatedAt=FROM_UNIXTIME({now})'"
    )
    print(f"\nSet {key}:")
    print(stdout.read().decode())
    print(stderr.read().decode())

# Vérifier les variables
stdin, stdout, stderr = client.exec_command(
    "docker exec appwrite-mariadb mysql -u root -p'appwrite-db-pass-2024' appwrite -e 'SELECT key, value FROM _1_variables WHERE resourceId=\"send-otp-whatsapp\"'"
)
print(f"\nVariables après mise à jour:\n{stdout.read().decode()}")

# Restart executor
stdin, stdout, stderr = client.exec_command("docker restart appwrite-executor")
print(f"\nRestart executor: {stdout.read().decode()}")

client.close()
